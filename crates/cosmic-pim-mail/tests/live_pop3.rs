...
fn a_second_pass_downloads_nothing_it_already_has() {
    ...
    let mut session = connect(&server);
    sync_inbox(&mut session, &mut store, &mut state, Retention::LeaveOnServer, 1_000)
        .expect("first pass");
...
fn deleting_when_fetched_happens_after_the_message_is_on_disk() {
    ...
    let retr = commands.iter().position(|c| c.starts_with("RETR")).expect("a RETR");
    let dele = commands.iter().position(|c| c.starts_with("DELE")).expect("a DELE");
    assert!(
        retr < dele,
...
fn an_account_read_twice_keeps_one_copy_per_message() {
    ...
    assert_eq!(outcome.fetched, 0, "a renumbered message was downloaded again");
    assert_eq!(
